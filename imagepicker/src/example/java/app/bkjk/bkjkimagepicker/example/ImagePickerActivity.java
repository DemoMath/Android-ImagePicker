package app.bkjk.bkjkimagepicker.example;
...
import app.bkjk.bkjkimagepicker.example.imageloader.PicassoImageLoader;
import app.bkjk.bkjkimagepicker.example.imageloader.UILImageLoader;
import app.bkjk.bkjkimagepicker.example.imageloader.XUtils3ImageLoader;
import app.bkjk.bkjkimagepicker.example.wxdemo.WxDemoActivity;
...
public class ImagePickerActivity extends AppCompatActivity implements SeekBar.OnSeekBarChangeListener, CompoundButton.OnCheckedChangeListener, View.OnClickListener {
    ...
    private IPImagePicker imagePicker;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imagePicker = IPImagePicker.getInstance();
        imagePicker.setIPImageLoader(new GlideImageLoader());
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_open_gallery:
                if (rb_uil.isChecked()) imagePicker.setIPImageLoader(new UILImageLoader());
                else if (rb_glide.isChecked()) imagePicker.setIPImageLoader(new GlideImageLoader());
                else if (rb_picasso.isChecked()) imagePicker.setIPImageLoader(new PicassoImageLoader());
                else if (rb_fresco.isChecked()) imagePicker.setIPImageLoader(new GlideImageLoader());
                else if (rb_xutils3.isChecked()) imagePicker.setIPImageLoader(new XUtils3ImageLoader());
                else if (rb_xutils.isChecked()) imagePicker.setIPImageLoader(new GlideImageLoader());
                ...
                if (rb_crop_square.isChecked()) {
                    imagePicker.setStyle(IPCropImageView.Style.RECTANGLE);
                    ...
                    imagePicker.setStyle(IPCropImageView.Style.CIRCLE);
                ...
                Intent intent = new Intent(this, IPImageGridActivity.class);
                intent.putExtra(IPImageGridActivity.EXTRAS_IMAGES,images);
    ...
    private class MyAdapter extends BaseAdapter {
        ...
        private List<IPImageItem> items;

        public MyAdapter(List<IPImageItem> items) {
            this.items = items;
        }

        public void setData(List<IPImageItem> items) {
            this.items = items;
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            imagePicker.getIPImageLoader().displayImage(ImagePickerActivity.this, getItem(position).path, imageView, size, size);
            return imageView;
        }
    }
}
