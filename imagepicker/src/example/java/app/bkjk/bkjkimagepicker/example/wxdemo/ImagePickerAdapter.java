...
public class ImagePickerAdapter extends RecyclerView.Adapter<ImagePickerAdapter.SelectedPicViewHolder> {
    ...
    private List<IPImageItem> mData;
    ...
    public void setImages(List<IPImageItem> data) {
        mData = new ArrayList<>(data);
        if (getItemCount() < maxImgCount) {
            mData.add(new IPImageItem());
            isAdded = true;
        } else {
            isAdded = false;
        }
    ...
    public List<IPImageItem> getImages() {
        //由于图片未选满时，最后一张显示添加图片，因此这个方法返回真正的已选图片
        if (isAdded) return new ArrayList<>(mData.subList(0, mData.size() - 1));
        else return mData;
    }

    public ImagePickerAdapter(Context mContext, List<IPImageItem> data, int maxImgCount) {
    ...
    public class SelectedPicViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void bind(int position) {
            ...
            //根据条目位置设置图片
            IPImageItem item = mData.get(position);
            if (isAdded && position == getItemCount() - 1) {
                iv_img.setImageResource(R.drawable.selector_image_add);
                clickPosition = WxDemoActivity.IMAGE_ITEM_ADD;
            } else {
                IPImagePicker.getInstance().getIPImageLoader().displayImage((Activity) mContext, item.path, iv_img, 0, 0);
